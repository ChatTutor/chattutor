...
class MessageDB:
    ...
    def get_user(self, user_id):
        with self.connect_to_messages_database() as con:
            cur = con.cursor()
            cur.execute(f"SELECT * FROM lusers WHERE id = '{user_id}'")
            users = cur.fetchall()
            return users

    def insert_user_to_course(self, user_id, course_id):
        with self.connect_to_messages_database() as con:
            cur = con.cursor()
            cur.execute(
                f"INSERT INTO ruserscourses (id, course_id) VALUES ('{user_id}', '{course_id}') ON DUPLICATE KEY UPDATE id='{user_id}', course_id='{course_id}'"
            )
            con.commit()
    ...
    def get_user_courses(self, user_id):
        with self.connect_to_messages_database() as con:
            cur = con.cursor()
            cur.execute(
                f"SELECT * FROM lcourses WHERE course_id IN (SELECT course_id FROM ruserscourses WHERE user_id = '{user_id}')"
            )
    ...
    def get_courses_sections(self, course_id):
        with self.connect_to_messages_database() as con:
            cur = con.cursor()
            cur.execute(
                f"SELECT * FROM lsections WHERE section_id IN (SELECT section_id FROM rsectionscourses WHERE course_id = '{course_id}')"
            )
    ...
    def insert_course(self, course_id, name, proffessor, mainpage, collectionname):
        with self.connect_to_messages_database() as con:
            cur = con.cursor()
            cur.execute(
                f"INSERT INTO lcourses (course_id, name, proffessor, mainpage, collectionname) VALUES ('{course_id}', '{name}', '{proffessor}', '{mainpage}', '{collectionname}') ON DUPLICATE KEY UPDATE course_id='{course_id}', name='{name}', proffessor='{proffessor}', mainpage='{mainpage}', collectionname='{collectionname}'"
            )
            con.commit()
    ...
    def establish_course_section_relationship(self, section_id, course_id):
        with self.connect_to_messages_database() as con:
            cur = con.cursor()
            cur.execute(
                f"INSERT INTO rsectionscourses (section_id, course_id) VALUES ('{section_id}', '{course_id}') ON DUPLICATE KEY UPDATE section_id='{section_id}', course_id='{course_id}'"
            )
            con.commit()
    ...
    def update_section_add_fromdoc(self, section_id, from_doc):
        with self.connect_to_messages_database() as con:
            cur = con.cursor()
            cur.execute(
                f"UPDATE lsections SET pulling_from = concat(pulling_from, '{'$'+from_doc}') WHERE section_id = '{section_id}'"
            )
            con.commit()
    ...
    def insert_config(
        ...
        with self.connect_to_messages_database() as con:
            ...
            """
            print(
                f"""INSERT INTO lconfigstokens (user, course_id, course_url, run_locally, test_mode, is_static, built_with, server_port, chattutor_server, token_id, use_as_default)
    ...
    def get_config_by_course_id(self, course_id: str):
        with self.connect_to_messages_database() as con:
            ...
            response = cur.execute(
                f"SELECT * FROM lconfigstokens WHERE course_id='{course_id}'"
            )
            messages_arr = cur.fetchall()
            return messages_arr
    ...
    def get_default_config_for_url(self, url: str):
        with self.connect_to_messages_database() as con:
            cur = con.cursor()
            response = cur.execute(
                f"SELECT * FROM lconfigstokens WHERE course_url='{url}' AND use_as_default=1"
            )
            messages_arr = cur.fetchall()
            return messages_arr
