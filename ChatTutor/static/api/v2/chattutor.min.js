...
headers.append('Access-Control-Allow-Origin', `${CHATTUTOR_SERVER}`);
...
const BASE_URL = testmode ? `${CHATTUTOR_SERVER}` : "https://chattutor.org"

    // URLs for production and debugging
const prodAskURL = new URL("https:/chattutor.org/ask");
const debugAskURL = new URL(`${CHATTUTOR_SERVER}/ask`);
...
var pulling_from_docs = []

async function setupEmbedMode_URL() {
    ...
    const response = await fetch(BASE_URL + "/get_section", {
        method: 'POST',
        headers: headers,
        body: JSON.stringify({
            collection: EMBEDDING_COLLECTION_NAME,
            section_id: original_file
        })
    })
    const data = await response.json()
    console.log(data["pulling_from"][0])
    pulling_from_docs = data["pulling_from"][0].split("$")
  }
...
function fetchClearCollection(collname) {
    ...
    fetch(BASE_URL + "/delete_uploaded_data", {
        method: 'POST',
        headers: headers,
...
function queryGPT(fromuploaded = false, uploaded_collection_name = EMBEDDING_COLLECTION_NAME) {
    ...
    if (embed_mode) {
        ...
        if (API_VERSION > 1) {
            args.from_doc = pulling_from_docs
            args.from_doc.append(original_file)
        } else {
