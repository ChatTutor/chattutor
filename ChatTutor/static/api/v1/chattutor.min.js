...
const BASE_URL = testmode ? "http://127.0.0.1:5000" : "https://chattutor.org";
...
function setupEmbedMode_URL() {
  ...
  charttutor_embed_elemment_andu.setAttribute(
    "style",
  ...
  if (
    BUILT_WITH == "JUPYTER-NOTEBOOK" &&
    RUN_LOCALLY == true &&
    IS_STATIC == true
  ) {
    const BUILD_HTML_JUPYTER_NOTEBOOK_FOLDER = "_build/html";
    let ind = path.indexOf(BUILD_HTML_JUPYTER_NOTEBOOK_FOLDER);
    let end = ind + BUILD_HTML_JUPYTER_NOTEBOOK_FOLDER.length;
...
function setProperties() {
  ...
  const interfaceObject =
    interfaceTheme === "normal"
      ? object.normalInterface_
      : object.imessageInterface_;
...
function clearFileInput(ctrl) {
  ...
  try {
    ctrl.parentNode.querySelector("span").innerHTML =
      `<span>Upload File</span>`;
  } catch (ex) {}
}
...
let selectedModel = "gpt-4-1106-preview"; //document.getElementById('modelDropdown').value
...
const selectUploadedCollection = document.getElementById(
  "selectUploadedCollection",
);
...
function setTheme(th) {
  setProperties();
  const _style =
    '"font-size: 15px !important; padding: 0 !important; margin: 0 !important; vertical-align: middle"';
...
function queryGPT(
  fromuploaded = false,
  uploaded_collection_name = EMBEDDING_COLLECTION_NAME,
) {
  ...
  if (selectUploadedCollection.options.length > 0) {
    selected_collection_name =
      selectUploadedCollection.options[selectUploadedCollection.selectedIndex]
        .value;
  }
  const args = {
  ...
  fetch(BASE_URL + "/ask", {
    ...
    .then((response) => {
      ...
      let elapsed_total = 0.0;
      let elapsed_time = -1.0;
      let processing_prompt_time = -1.0;
      function read() {
        ...
        reader
          ...
          .then(({ done, value }) => {
            ...
            const messages = strValue
              .split("\n\n")
              .filter(Boolean)
              .map((chunk) => JSONparse(chunk.split("data: ")[1]));
            let message;
            for (var messageIndex in messages) {
              message = messages[messageIndex];
              if (stopGeneration === false) {
                if (message.message.elapsed_time) {
                  elapsed_time = message.message.elapsed_time
                  elapsed_total += elapsed_time
                  console.log(elapsed_time)
                }
                if (message.message.processing_prompt_time) {
                  processing_prompt_time = message.message.processing_prompt_time
                  console.log(processing_prompt_time)
                }
                if (message.message.valid_docs) {
                  context_documents = message.message.valid_docs;
                  console.log(context_documents);
                }
                const contentToAppend = message.message.content
              ...
              if (stopGeneration === true) {
                ...
                // );
                scrollHelper.scrollIntoView();
                updateLastMessage(accumulatedContent, id_2);
            ...
            console.log("Elapsed time:", elapsed_total, "Prep time", processing_prompt_time)
          })
          .catch((err) => {
...
function setLatMessageHeader(
  context_documents,
  lastMessageIdParam,
  add = true,
) {
  if (context_documents == false) return "";
...
function updateLastMessage(newContent, msg_db_id) {
  if (lastMessageId) {
    ...
    if (lastMessageElement) {
      ...
      document.querySelector(`#${lastMessageId} .msg-text`).innerHTML =
        newContentFormatted;

        document.querySelector(`#${lastMessageId} .msg-id`).innerHTML =
        msg_db_id;
    } else {
      console.error("Cannot find the .msg-text element to update.");
    }
...
function send_feedback(e, content) {
  // here
  e = e || window.event;
  var targ = e.target || e.srcElement || e;
  if (targ.nodeType == 3) targ = targ.parentNode; // defeat Safari bug
  var id_elem = targ.parentElement.children[0];
...
function addMessage(
    ...
    <div class="msg-bgd">
      ...
      <div class="msg-bubble">
        ...
        <div class="feedback row" style="display: ${feedback_display};">
          ...
          <div class="fit feedback-btn" onclick="send_feedback(this, 'positive')">
            <i class="large material-icons">👍🏻</i>
          </div>
          <div class="fit feedback-btn" onclick="send_feedback(this, 'negative')">
            <i class="large material-icons">👎🏻</i>
          </div>
        </div>
      </div>
  ...
  if (updateConversation) {
    conversation.push({ role: role, content: message, message_id: msg_db_id });
...
function setupEmbedMode() {
  ...
  const download_original = document.querySelectorAll(
    '[title="Download source file"]',
  )[0];
  original_file = download_original
    .getAttribute("href")
    .slice(download_original.getAttribute("href").lastIndexOf("/") + 1);
}
...
function uploadFile() {
  ...
  sendUploadedZipButton.querySelector("span").innerHTML =
...
function addCollectionToFrontEnd(created_collection_name) {
  ...
  localStorage.setItem(
    ...
    JSON.stringify(uploadedCollections),
  );
...
function validateUrls() {
  let arr = [];
  if (
    msgerInput.value.includes(":url:") ||
    msgerInput.value.includes(":file:")
  ) {
  } else {
    return undefined;
  }
...
function getTextSegments(element) {
  ...
  Array.from(element.childNodes).forEach((node) => {
    switch (node.nodeType) {
      ...
      default:
        throw new Error(
          `Unexpected node type: ${node.nodeType} ${Node.TEXT_NODE}`,
        );
    }
  });
  return textSegments;
...
function renderText(text) {
  ...
  words.forEach((f) => {
    if (f == ":url:" || f == ":file:" || windex == words.length - 1) {
      if (el !== undefined && lastf !== undefined) arr.push(el);
